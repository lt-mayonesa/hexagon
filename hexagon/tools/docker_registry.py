...
def docker_registry(registry_host):
    ...
    def get_authenticated(_url, j_path=None):
        __x = requests.get(_url, headers={
            'Authorization': f'Basic {auth}'
        }).json()
        return __x[j_path] if j_path else __x

    image = tracer.tracing(_image or inquirer.fuzzy(
        message="¿Qué imagen estás buscando?",
        choices=lambda _: get_authenticated(f'https://{registry_host}/v2/_catalog', 'repositories'),
    ).execute())

    tag = tracer.tracing(_filter or inquirer.fuzzy(
        message="¿Qué tag?",
        choices=lambda _: get_authenticated(f'https://{registry_host}/v2/{image}/tags/list', 'tags')
    ).execute())
    ...
    manifest = inquirer.confirm(
        message='¿Te gustaría ver el manifest?',
        default=False
    ).execute()
