...
- **CRITICAL: Always run Black and Flake8 before committing**: Code quality checks are mandatory for ALL code changes. Run these commands on the entire project before considering work complete:
  ```bash
  pipenv run black .
  pipenv run flake8 .
  ```
